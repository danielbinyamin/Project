...
private void initialize() {
	...
	btnAddCombinedDb.addActionListener(new ActionListener() {
		public void actionPerformed(ActionEvent e) {
			...
			String url = "jdbc:mysql://" + ip + ":" + port + "/" + db;	//"jdbc:mysql://"+_ip+":3306/oop_course_ariel"
			...
			if(!dbWithAboveDetailsAlreadyLoaded) {
				try {
					Connection con = _programCore.addCombinedDB(url, table, usermame, password);
					...
					try {
						...
						System.out.println("error adding DB to programCore.");
					}
				}
