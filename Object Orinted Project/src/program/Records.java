...
public class Records  {
	 ...
	 * @throws Exception 
	 */
	public void loadRecordsFromDB(ResultSet db){
		//move line by line
		try {
			while (db.next()){
				//date
				String dateString = db.getString("time");
				Calendar date = Calendar.getInstance();
				SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
				date.setTime(sdf.parse(dateString));
				String device = db.getString("device"); 

				//id
				String id = db.getString("id");
				//location
				double lat = db.getDouble("lat");
				double lon = db.getDouble("lon");
				double alt = db.getDouble("alt");
				//wifi-list
				ArrayList<Wifi> wifiList = new ArrayList<>();
				String ssid, mac;
				int freq, signal;
				for (int wifiIndex = 0; wifiIndex < db.getInt("number_of_ap"); wifiIndex = wifiIndex + 4) {
					ssid = "Unknown SSID";
					mac = db.getString("mac" + wifiIndex);
					freq = -1;
					signal = db.getInt("rssi" + wifiIndex);
					Wifi currentWifi = new Wifi(ssid,mac,freq,signal);
					wifiList.add(currentWifi);
				}
				//create singleRecord and add it to list
				SingleRecord currentRecord = new SingleRecord(id, wifiList, date, lon, lat, alt);
				currentRecord.set_id(device);
				_records.add(currentRecord);
			}
		}
